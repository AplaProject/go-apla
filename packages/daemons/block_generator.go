...
func BlockGenerator(ctx context.Context, d *daemon) error {
	...
	err = p.AllTxParser()
	endTime := time.Now()
	fmt.Println("all tx parser:", endTime.Sub(startTime))
	if err != nil {
		return err
	}
	...
	endTime = time.Now()
	fmt.Println("Get all transactions: ", endTime.Sub(startTime))
	...
	endTime = time.Now()
	fmt.Println("block generated by: ", endTime.Sub(startTime))
	return err
}
