...
func QueueParserBlocks(ctx context.Context, d *daemon) error {
	...
	// check qblock node != myself
	cfg, err := model.GetConfig()
	if err != nil {
		return utils.ErrInfo("unable to get config!")
	}
	if queueBlock.FullNodeID == cfg.KeyID {
	...
	return UpdateChain(ctx, d, host, blockID, "rollback_blocks_1")
}
