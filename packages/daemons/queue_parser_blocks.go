...
func QueueParserBlocks(ctx context.Context, d *daemon) error {
	...
	item, err := queue.ValidateBlockQueue.Dequeue()
	if err != nil {
		log.WithFields(log.Fields{"type": consts.QueueError, "error": err}).Error("getting block from validate queue")
		return err
	}
	queueBlock := blockchain.Block{}
	if err := queueBlock.Unmarshal(item.Value); err != nil {
		return err
	}
	// check if the block gets in the rollback_blocks_1 limit
	if queueBlock.Header.BlockID > infoBlock.Header.BlockID+syspar.GetRbBlocks1() {
		return utils.ErrInfo("rollback_blocks_1")
	}

	// is it old block in queue ?
	if queueBlock.Header.BlockID <= infoBlock.Header.BlockID {
		return utils.ErrInfo(fmt.Errorf("old block %d <= %d", queueBlock.Header.BlockID, infoBlock.Header.BlockID))
	}
	if queueBlock.Header.KeyID == conf.Config.KeyID {
		d.logger.WithFields(log.Fields{"type": consts.DuplicateObject}).Debug("queueBlock generated by myself", queueBlock.Header.BlockID)
		return utils.ErrInfo(fmt.Errorf("queueBlock generated by myself: %d", queueBlock.Header.BlockID))
	}

	nodeHost, err := syspar.GetNodeHostByPosition(queueBlock.Header.KeyID)
	if err != nil {
		return utils.ErrInfo(err)
	}
	blockID := queueBlock.Header.BlockID
