...
*/
		...
		// откатим transactions_candidateBlock
		...
		for rows.Next() {
			...
			exists, err := d.Single("SELECT hash FROM transactions_candidateBlock WHERE hex(hash) = ?", hashMd5).String()
			if err != nil {
				rows.Close()
				if d.dPrintSleep(utils.ErrInfo(err), d.sleepTime) {
					break BEGIN
			...
			if len(exists) == 0 {
				err = d.ExecSql(`INSERT INTO transactions_candidateBlock (hash, data, type, user_id, third_var) VALUES ([hex], [hex], ?, ?, ?)`,
		...
		*/
		...
		//err = d.ExecSql("DELETE FROM candidateBlock WHERE block_id = ?", newBlockId)
		...
		err = d.ExecSql(`INSERT INTO candidateBlock (block_id, time, level, user_id, header_hash, signature, mrkl_root) VALUES (?, ?, ?, ?, [hex], [hex], [hex])`,
		...
		/// #######################################
		// Отмечаем транзакции, которые попали в transactions_candidateBlock
		// Пока для эксперимента
		// если не отмечать, то получается, что и в transactions_candidateBlock и в transactions будут провернные тр-ии, которые откатятся дважды
		if len(usedTransactions) > 0 {
			...
			// для теста удаляем, т.к. она уже есть в transactions_candidateBlock
