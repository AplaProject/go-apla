...
func (req *GetBodiesRequest) Read(r io.Reader) error {
	...
	if err != nil {
		log.WithFields(log.Fields{"type": consts.IOError, "error": err}).Error("on reading GetBodiesRequest")
		return err
	}
	req.BlockHash = h
...
func (req *GetBodiesRequest) Write(w io.Writer) error {
	if err := writeSliceWithSize(w, req.BlockHash, consts.HashSize); err != nil {
		log.WithFields(log.Fields{"type": consts.IOError, "error": err}).Error("on sending GetBodiesRequest")
		return err
	}
...
func (req *ConfirmRequest) Read(r io.Reader) error {
	h, err := readSliceWithSize(r, consts.HashSize)
	if err != nil {
		log.WithFields(log.Fields{"type": consts.IOError, "error": err}).Error("on reading ConfirmRequest")
		return err
	}
	req.BlockHash = h
	return nil
...
func (req *ConfirmRequest) Write(w io.Writer) error {
	if err := writeSliceWithSize(w, req.BlockHash, consts.HashSize); err != nil {
		log.WithFields(log.Fields{"type": consts.IOError, "error": err}).Error("on sending ConfirmRequest")
		return err
	}
