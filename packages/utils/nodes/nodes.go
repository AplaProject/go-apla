...
func main() {
	...
	for {
		select {
		case <-ticker.C:
			if err := MoneyTransfer(); err != nil {
				fmt.Println(err)
				return
			} else {
				counter++
				fmt.Println("money transfered", counter, "times")
			}
		case <-timer.C:
			return
		}
...
func MoneyTransfer() error {
	form := url.Values{
		"Recipient": {converter.AddressToString(542353610328569127)},
		"Amount":    {"1"},
	}

	return postTx("MoneyTransfer", &form)
}
