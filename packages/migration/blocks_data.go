...
Form(){
	Div(clearfix){
		...
		(next_id('1_blocks'), 'export_info', 'DBFind(Name: buffer_data, Source: src_buffer).Columns("value->app_id,value->app_name,value->menu_name,value->menu_id,value->count_menu").Where({key:export, member_id: #key_id#}).Vars(buffer)
