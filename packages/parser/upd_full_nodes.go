...
func (p *UpdFullNodesParser) Init() error {
	if p.BlockData != nil && p.BlockData.Version == 0 {
		log.Debugf("use old UpdFullNode parser")
		...
		if len(oldSlice) < 6 {
			return fmt.Errorf("bad transaction format (got %d values)", len(oldSlice))
		}

		p.UpdFullNodes = &tx.UpdFullNodes{
			Header: tx.Header{
				Type:          int(p.DataType),
				Time:          converter.BytesToInt64(oldSlice[2]),
				UserID:        converter.BytesToInt64(oldSlice[3]),
				StateID:       converter.BytesToInt64(oldSlice[4]),
				BinSignatures: oldSlice[5],
			},
		}
		log.Debugf("header: %+v", p.UpdFullNodes.Header)
		return nil
	}
...
func (p *UpdFullNodesParser) Validate() error {
	...
	if len(wallet.NodePublicKey) == 0 {
		return utils.ErrInfoFmt("len(nodePublicKey) = 0")
	}
	CheckSignResult, err := utils.CheckSign([][]byte{wallet.NodePublicKey}, p.UpdFullNodes.ForSign(), p.UpdFullNodes.BinSignatures, true)
	if err != nil {
		return p.ErrInfo(err)
	}
	if !CheckSignResult {
...
func (p *UpdFullNodesParser) Action() error {
	...
	// we don't want update feul rate for old style transactions
	if p.BlockData != nil && p.BlockData.Version == 0 {
		return nil
	}
