...
func (p *Parser) RestoreAccess() error {
	...
	_, _, err = p.selectiveLoggingAndUpd([]string{"close"}, []interface{}{"1"}, "system_restore_access", []string{"state_id"}, []string{converter.Int64ToStr(p.TxMaps.Int64["state_id"])}, true)
	if err != nil {
		return p.ErrInfo(err)
	}
	return nil
