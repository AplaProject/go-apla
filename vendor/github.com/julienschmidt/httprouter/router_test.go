...
func TestRouter(t *testing.T) {
	...
	router.Handle("GET", "/user/:name", func(w http.ResponseWriter, r *http.Request, ps Params) {
		routed = true
		fmt.Println(`GET OK`, r.Method, ps.ByName(`name`))
		want := Params{Param{"name", "gopher"}}
		if !reflect.DeepEqual(ps, want) {
			t.Fatalf("wrong wildcard values: want %v, got %v", want, ps)
		}
	})
	router.Handle("POST", "/user/:name", func(w http.ResponseWriter, r *http.Request, ps Params) {
		routed = true
		fmt.Println(`POST OK`, r.Method, ps.ByName(`name`))
		want := Params{Param{"name", "myname"}}
		if !reflect.DeepEqual(ps, want) {
			t.Fatalf("wrong wildcard values: want %v, got %v", want, ps)
		}
	})
	...
	req, _ = http.NewRequest("POST", "/user/myname", nil)
	router.ServeHTTP(w, req)
